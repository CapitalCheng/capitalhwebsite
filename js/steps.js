 ...
 *
 * Each animation:
 *  - Injects its own canvas into .step-num (hides the text number)
 *  - Starts only when scrolled into view (IntersectionObserver)
 *  - Pauses when off-screen (CPU/battery)
 *  - Throttles to 30fps on low-end devices
 *
 * Expects:  .step elements with .step-num children (4 total)
 * Load via: <script src="js/steps.js"></script>
 */
(function () {
  ...
  // Inject canvas into a .step-num, hide the text, return {canvas, ctx, W, H}
  function injectCanvas(stepEl) {
    ...
    const W    = numEl.offsetWidth  || 72;
    const H    = numEl.offsetHeight || 72;
    canvas.width  = W * DPR;
    canvas.height = H * DPR;
  ...
  function initMap(stepEl) {
    ...
    keys.forEach(k => { k.total = k.fadeIn + k.hold + k.fadeOut + rnd(0.5, 2); k.life = k.phaseOffset; });
    ...
    let lastTs = null, lastFrameTs = 0, visible = false, running = false;
    function tick(ts) {
      ...
      keys.forEach(k => {
        ...
        if (k.life < k.fadeIn) {
          ...
          alpha = 1; pulse = (Math.sin((k.life - k.fadeIn) * 3) + 1) * 0.5;
        } else {
        ...
        ctx.fillStyle = `rgba(${CTA[0]},${CTA[1]},${CTA[2]},${alpha*0.9})`; ctx.fill();
      });
    }
  ...
  function initDesign(stepEl) {
    ...
    function circlePos(i,n){const a=(i/n)*Math.PI*2-Math.PI/2;return{x:CX+Math.cos(a)*R,y:CY+Math.sin(a)*R};}
    function triPos(i,n){
      const s=Math.floor(i/(n/3)),p=(i%(n/3))/(n/3);
      const pts=[{x:CX,y:CY-R},{x:CX+R*0.87,y:CY+R*0.5},{x:CX-R*0.87,y:CY+R*0.5}];
      const nx=pts[(s+1)%3]; return{x:lerp(pts[s].x,nx.x,p),y:lerp(pts[s].y,nx.y,p)};
    }
    function sqPos(i,n){
      const s=Math.floor(i/(n/4)),p=(i%(n/4))/(n/4),r=R*0.82;
      const pts=[{x:CX-r,y:CY-r},{x:CX+r,y:CY-r},{x:CX+r,y:CY+r},{x:CX-r,y:CY+r}];
      const nx=pts[(s+1)%4]; return{x:lerp(pts[s].x,nx.x,p),y:lerp(pts[s].y,nx.y,p)};
    }
    const shapes=[circlePos,triPos,sqPos];
    const HOLD=2.2,TRANS=1.0,CYCLE=HOLD+TRANS;
    const dots=Array.from({length:N},()=>({x:CX+(Math.random()-0.5)*20,y:CY+(Math.random()-0.5)*20,vx:(Math.random()-0.5)*0.3,vy:(Math.random()-0.5)*0.3,r:2.2}));

    let t=0,lastTs=null,lastFrameTs=0,visible=false,running=false;
    function tick(ts){
      requestAnimationFrame(tick);
      if(!visible)return;
      if(FRAME_MS&&ts-lastFrameTs<FRAME_MS)return;
      lastFrameTs=ts;
      const dt=Math.min((ts-(lastTs||ts))/1000,0.05); lastTs=ts; t+=dt;
      ctx.clearRect(0,0,W,H);
      ...
      dots.forEach((d,i)=>{
        ...
        if(inTrans){d.vx+=(Math.random()-0.5)*0.15;d.vy+=(Math.random()-0.5)*0.15;d.vx*=0.95;d.vy*=0.95;d.x+=d.vx;d.y+=d.vy;}
        else{d.x+=(tx-d.x)*0.06+(Math.random()-0.5)*0.3;d.y+=(ty-d.y)*0.06+(Math.random()-0.5)*0.3;}
  ...
  function initImplement(stepEl) {
    ...
    const landed=[];
    let falling=null,nextCol=0,nextRow=0,resetTimer=0,lastTs=null,lastFrameTs=0,visible=false,running=false;

    function spawnFalling(){
      if(nextRow>=ROWS){resetTimer=1.8;return;}
      const col=nextCol%COLS;
      falling={x:rnd(4,W-4),y:rnd(4,swarmH*0.8),vx:rnd(-0.5,0.5),vy:0,
        tx:ox+col*STEP+DOT/2,ty:oy+nextRow*STEP+DOT/2,alpha:0};
      nextCol++; if(nextCol%COLS===0)nextRow++;
    }
    spawnFalling();
    ...
    function tick(ts){
      ...
      swarm.forEach(d=>{
        ...
        d.vx*=0.97;d.vy*=0.97;
        if(d.x<2||d.x>W-2)d.vx*=-1; if(d.y<2||d.y>swarmH-2)d.vy*=-1;
        ctx.beginPath();ctx.arc(d.x,d.y,d.r,0,Math.PI*2);
        ctx.fillStyle=`rgba(${GRAY[0]},${GRAY[1]},${GRAY[2]},${d.alpha})`;ctx.fill();
      });

      if(resetTimer>0){resetTimer-=dt;if(resetTimer<=0){landed.length=0;nextCol=0;nextRow=0;falling=null;spawnFalling();}}
      ...
      if(falling){
        ...
        if(dist<3){landed.push({x:falling.tx,y:falling.ty});falling=null;setTimeout(spawnFalling,180);}
      }
    }
    const io=new IntersectionObserver(e=>{
      visible=e[0].isIntersecting;
  ...
  function initEmbed(stepEl) {
    ...
    const dots=Array.from({length:N},(_,i)=>{
      ...
      return{
        ...
        vx:i===0?0:(Math.random()-0.5)*0.25,vy:i===0?0:(Math.random()-0.5)*0.25,
        r:i===0?2.6:rnd(1.0,1.9),
        baseAlpha:i===0?1:rnd(0.25,0.42),
        redness:i===0?1:0,dist,
    ...
    let waveR=0,t=0,lastTs=null,lastFrameTs=0,visible=false,running=false;
    ...
    function tick(ts){
      ...
      lastFrameTs=ts;
      const dt=Math.min((ts-(lastTs||ts))/1000,0.05); lastTs=ts; t+=dt;
      waveR+=dt*14; if(waveR>maxDist+20)waveR=0;
      ctx.clearRect(0,0,W,H);
      ...
      dots.forEach(d=>{
        if(!d.isCenter){
          ...
          d.vx*=0.97;d.vy*=0.97;
          d.x+=d.vx;d.y+=d.vy;
          if(d.x<2||d.x>W-2)d.vx*=-1;if(d.y<2||d.y>H-2)d.vy*=-1;
